# Registration, authentication and sub-wallets

Registering a wallet (random, deterministic, vault-bound), the three auth modes, creating sub-agents, and recovering a lost key.
...
**Custody wallets** (created via `POST /register` with a `wk_` key) don't need NEAR signatures for sub-agents. Use `PUT /wallet/v1/api-key` with your Bearer token — see the "Create Sub-Agents" section in this file.
